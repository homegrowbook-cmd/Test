...
export default function RunDetailClient() {
  ...
  const handleDelete = async () => {
    if (!confirm('Möchten Sie dieses Tagebuch wirklich löschen?')) {
      return;
    }
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Fehler beim Löschen des Tagebuchs');
    }
  };
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
          {error || 'Tagebuch nicht gefunden'}
        </div>
        <div className="mt-4">
          <Link href="/diaries" className="btn-primary">
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <div className="flex gap-6 text-sm">
          ...
          <span>💬 {run._count?.comments || 0} Kommentare</span>
          <span>📝 {run._count?.entries || 0} Einträge</span>
        </div>
      </div>
      ...
      <div className="card mb-8">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          {run.strainType && (
            <div>
              <label className="text-sm text-gray-600 dark:text-gray-400">Typ</label>
          ...
          {run.lightType && (
            <div>
              <label className="text-sm text-gray-600 dark:text-gray-400">Licht</label>
          ...
          {run.nutrients && (
            <div>
              <label className="text-sm text-gray-600 dark:text-gray-400">Nährstoffe</label>
              <p className="font-semibold">{run.nutrients}</p>
            </div>
          )}

          <div>
            <label className="text-sm text-gray-600 dark:text-gray-400">Gestartet</label>
            <p className="font-semibold">
          ...
          {run.endDate && (
            <div>
              <label className="text-sm text-gray-600 dark:text-gray-400">Beendet</label>
              <p className="font-semibold">
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold">Einträge</h2>
          {isOwner && (
        ...
        {entries.length === 0 ? (
          <div className="card text-center py-12">
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              Noch keine Einträge. Beginne dein Grow zu dokumentieren!
            </p>
            {isOwner && (
              <Link href={`/runs/${run.id}/entries/new`} className="btn-primary">
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Tag {entry.dayNumber} • Woche {entry.weekNumber}
                      </p>
                    </div>
